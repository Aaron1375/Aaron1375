						...
						<div class="form-inline">
							<div class="form-group">
								<label>Video Title <select name="" id=""
									class="form-control ml-3">
									<c:forEach var="item1" items="${shareList }">
										<option value="${item1.videoId }" ${item1.videoId == userShareVideo?'selected':'' }>${item1.videoId }</option>
									</c:forEach>
								</select>
								</label>
								<button class="btn btn-info ml-2">Report</button>
				...
				<div class="row">
					<div class="col mt-3">
						<table class="table table-border">
							...
							<c:forEach var="item2" items="${shareUser }">
							<tr>
								<td>${item.username }</td>
								<td>${item.videoId }</td>
								<td>${item.emails }</td>
